<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.apartments.update', $apartment->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        <div class="mb-4">
                ...
                <select name="project_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($projects as $option)
                        <option value="{{ $option->id }}" {{ $apartment->project_id == $option->id ? 'selected' : '' }}>{{ $option->name }}</option>
                    @endforeach
                </select>
                @error('project_id')
                ...
                <input type="text" name="type" value="{{ old('type', $apartment->type) }}" class="w-full border border-gray-300 rounded p-2">
                @error('type')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="appendix" value="1" class="mr-2" {{ $apartment->appendix ? 'checked' : '' }}>
                    @lang('site.appendix')
                </label>
                @error('appendix')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <input type="text" name="code" value="{{ old('code', $apartment->code) }}" class="w-full border border-gray-300 rounded p-2">
                @error('code')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.room_count')</label>
                <input type="number" name="room_count" value="{{ old('room_count', $apartment->room_count) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('room_count')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.area')</label>
                <input type="number" name="area" value="{{ old('area', $apartment->area) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('area')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="number" name="price" value="{{ old('price', $apartment->price) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('price')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.price_bank')</label>
                <input type="number" name="price_bank" value="{{ old('price_bank', $apartment->price_bank) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('price_bank')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="file" name="img" accept="image/*" class="w-full border border-gray-300 rounded p-2">                @isset($apartment->img)
