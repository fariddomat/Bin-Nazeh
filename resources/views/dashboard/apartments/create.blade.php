<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.apartments.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
                        <div class="mb-4">
                ...
                <input type="number" name="room_count" value="{{ old('room_count') }}" class="w-full border border-gray-300 rounded p-2" >
                @error('room_count')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.area')</label>
                <input type="number" name="area" value="{{ old('area') }}" class="w-full border border-gray-300 rounded p-2" >
                @error('area')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="number" name="price_bank" value="{{ old('price_bank') }}" class="w-full border border-gray-300 rounded p-2" >
                @error('price_bank')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
