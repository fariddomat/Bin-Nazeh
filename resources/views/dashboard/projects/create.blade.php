<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.projects.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="floors_count" value="{{ old('floors_count') }}"
                    class="w-full border border-gray-300 rounded p-2">
                @error('floors_count')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="file" name="logo" accept="image/png,image/jpeg" class="w-full border border-gray-300 rounded p-2">
                @error('logo')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status" class="w-full border border-gray-300 rounded p-2">
                    ...
                    <option value="available" {{ old('status') == 'available' ? 'selected' : '' }}>متاح للحجز</option>
                    <option value="under_construction" {{ old('status') == 'under_construction' ? 'selected' : '' }}>تحت الإنشاء</option>
                    <option value="ready" {{ old('status') == 'ready' ? 'selected' : '' }}>جاهز</option>
                ...
                <select name="project_category_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($projectCategories as $option)
                        <option value="{{ $option->id }}"
                            {{ old('project_category_id') == $option->id ? 'selected' : '' }}>{{ $option->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="sort_id" value="{{ old('sort_id') }}"
                    class="w-full border border-gray-300 rounded p-2">
                @error('sort_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
