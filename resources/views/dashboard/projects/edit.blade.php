<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.projects.update', $project->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.name')</label>
                <input type="text" name="name" value="{{ old('name', $project->name) }}" class="w-full border border-gray-300 rounded p-2">
                @error('name')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.slug')</label>
                <input type="text" name="slug" value="{{ old('slug', $project->slug) }}" class="w-full border border-gray-300 rounded p-2">
                @error('slug')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.date_of_build')</label>
                <input type="date" name="date_of_build" value="{{ old('date_of_build', $project->date_of_build) }}" class="w-full border border-gray-300 rounded p-2">
                @error('date_of_build')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="text" name="scheme_name" value="{{ old('scheme_name', $project->scheme_name) }}" class="w-full border border-gray-300 rounded p-2">
                @error('scheme_name')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.floors_count')</label>
                <input type="number" name="floors_count" value="{{ old('floors_count', $project->floors_count) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('floors_count')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="file" name="img" accept="image/*" class="w-full border border-gray-300 rounded p-2">                @isset($project->img)
                ...
                <input type="file" name="cover_img" accept="image/*" class="w-full border border-gray-300 rounded p-2">                @isset($project->cover_img)
                ...
                <select name="status" class="w-full border border-gray-300 rounded p-2">
                    ...
                    <option value="not_started" {{ old('status', $project->status) == 'not_started' ? 'selected' : '' }}>not_started</option>
                ...
                <input type="number" name="status_percent" value="{{ old('status_percent', $project->status_percent) }}" class="w-full border border-gray-300 rounded p-2" >
                ...
                <select name="project_category_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($projectCategories as $option)
                        <option value="{{ $option->id }}" {{ $project->project_category_id == $option->id ? 'selected' : '' }}>{{ $option->name }}</option>
                    @endforeach
                </select>
                ...
                <input type="number" name="sort_id" value="{{ old('sort_id', $project->sort_id) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('sort_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.images')</label>
                <input type="file" name="images[]" accept="image/*" multiple class="w-full border border-gray-300 rounded p-2">                @if(!empty($project->images))
                    <div class="mt-2 flex flex-wrap">
                        @foreach(json_decode($project->images, true) ?? [] as $image)
                            <img src="{{ Storage::url($image) }}" alt="images" class="w-16 h-16 mr-2 rounded">
                        @endforeach
                    </div>
                @endif                @error('images')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
